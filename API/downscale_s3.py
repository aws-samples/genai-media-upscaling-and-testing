...
def sageMakerRetrieveUpscale(s3_bucket:str, endpoint: str, s3_key:str):
    ...
    try: 
        ...
        sageMakerRT = SagemakerRTUpscaleProvider(s3_bucket, endpoint)
...
def disect_request_store(request):
    ...
    if upscaleMethod == "sageMakerRT":
        ...
        try: 
            status = sageMakerDownscaleStore(s3_bucket=s3_bucket, endpoint=endpoint, s3_key=s3_key_name, image_data=image_data)                
